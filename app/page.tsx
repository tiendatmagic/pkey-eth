...
const humanizeDuration = (seconds: number, t: any) => {
  if (seconds > 200 * 365.25 * 24 * 3600 || seconds === -Infinity) {
    return t.thousandsOfYears || "Thousands of years";
  }
  ...
  if (d > 0) return `${d} ${t.days || "days"}`;
  if (h > 0) return `${h} ${t.hours || "hours"}`;
  if (m > 0) return `${m} ${t.minutes || "minutes"}`;
  return `${s} ${t.seconds || "seconds"}`;
};
...
export default function Home() {
  const [conditions, setConditions] = useState<
    { prefix: string; suffix: string; id: number }[]
  ...
  const [attemptsAtLastFind, setAttemptsAtLastFind] = useState(0);

  const [toasts, setToasts] = useState<{ id: number; message: string }[]>([]);
  const [isDark, setIsDark] = useState(true);

  const [lang, setLang] = useState<Language>("en");
  const [exampleHex, setExampleHex] = useState(
    "fa5ccC7a0E1C9b2d8f9B1a4C9A28D90FB2A28D90FB".slice(0, 40),
  );

  const [genMode, setGenMode] = useState<"privateKey" | "seedPhrase">(
    "privateKey",
  );
  ...
  useEffect(() => {
    ...
    const len = Math.max(
      0,
      40 - (firstCond.prefix.length + firstCond.suffix.length),
    );
  ...
  useEffect(() => {
    ...
    let lastFlushTick = performance.now();
    ...
    const uiRenderLoop = () => {
      ...
      if (
        foundWalletsBufferRef.current.length > 0 &&
        (now - lastFlushTick > 500 || !running)
      ) {
        lastFlushTick = now;
  ...
  const handleWorkerMessage = (data: any) => {
    ...
    if (data.address) {
      const target =
        walletCount === ""
          ? MAX_WALLETS
          : Math.min(Number(walletCount), MAX_WALLETS);
      ...
      setAttemptsAtLastFind(newAttempts);
      ...
      const activeConditions = conditions.filter((c) => c.prefix || c.suffix);
      for (const c of activeConditions) {
        const d = computeDifficulty(c.prefix, c.suffix, checksum);
        totalInvDifficulty += 1 / d;
      }
      const diff = totalInvDifficulty > 0 ? 1 / totalInvDifficulty : 1;

      if (diff >= 65536) {
        import("canvas-confetti").then((module) => {
          const blast = module.default || module;
      ...
      if (foundCountRef.current >= target) {
        const duration = Math.max(0.001, (performance.now() - startTickRef.current) / 1000);
        setTimeElapsed(duration);
        setKeysPerSec(Math.round(attemptsRef.current / duration));
  ...
  const startGen = () => {
    ...
    const filteredConditions = cleanConditions.filter(
      (c) => c.prefix || c.suffix,
    );
  ...
  const handleDownloadKeystore = async () => {
    if (
      !keystorePassword ||
      isEncrypting ||
      (keystoreMode === "single" && !selectedWallet)
    )
      return;
    ...
    setEncryptProgressText(
      keystoreMode === "single"
        ? t.encrypting
        : `${t.encrypting} 0/${foundWallets.length}`,
    );
      ...
      try {
        if (keystoreMode === "single" && selectedWallet) {
          ...
          for (let i = 0; i < foundWallets.length; i++) {
            ...
            setEncryptProgressText(
              `${t.encrypting} ${i + 1}/${foundWallets.length} (Heavy CPU)`,
            );
            ...
            const addressPart = wallet.address.toLowerCase().replace("0x", "");
            zip.file(
              `UTC--${new Date().toISOString().replace(/:/g, "-")}--${addressPart}.json`,
              json,
            );
          }
  ...
  const handleDownloadSeed = (wallet: any) => {
    ...
    if (wallet.mnemonic)
      content += `Your mnemonic phrase: ${wallet.mnemonic}\n`;
  ...
  const validConditions = conditions.filter(
    (c) =>
      (c.prefix || c.suffix) &&
      (!c.prefix || /^[0-9a-fA-F]+$/.test(c.prefix)) &&
      (!c.suffix || /^[0-9a-fA-F]+$/.test(c.suffix)),
  );
  const isInputValid =
    conditions.length > 0 &&
    conditions.every(
  ...
  const computeConditionsDifficulty = () => {
    let totalInvDifficulty = 0;
    const activeConditions = conditions.filter((c) => c.prefix || c.suffix);
    if (activeConditions.length === 0) return 1;
  ...
  const prob50Addresses =
    difficulty > 1
      ? Math.floor(Math.log(0.5) / Math.log(1 - 1 / difficulty))
      : 0;
  const time50 =
    keysPerSec > 0 && prob50Addresses > 0
      ? humanizeDuration(prob50Addresses / keysPerSec, t)
      : t.na;
  const attemptsSinceLastFind = Math.max(0, attempts - attemptsAtLastFind);
  const currentProbability = !isInputValid
    ? 0
    : difficulty <= 1
    ? (running ? 100 : 0)
    : (running
        ? Math.max(
            ...
            Math.min(
              ...
              Math.round(
                10000 *
                  computeProbability(difficulty, attemptsSinceLastFind),
              ) / 100,
            ),
          )
        : 0);
  const probabilityLabel =
    currentProbability === 0 && running && attemptsSinceLastFind > 0
      ? "<0.01%"
      : `${currentProbability.toFixed(2)}%`;

  const displayDifficulty = !isInputValid
    ? t.na
    : difficulty === 1
      ? 1
      : formatNum(difficulty.toFixed(2).replace(/\.00$/, ""));
  const displayAdresses50 =
    !isInputValid || difficulty === 1
      ? t.na
      : prob50Addresses === -Infinity
        ? t.nearlyImpossible
        : formatNum(prob50Addresses) + ` ${t.addresses}`;
  const displayTime50 =
    !isInputValid || difficulty === 1
      ? t.na
      : keysPerSec
  ...
  const codeBox = isDark
    ...
    : "bg-slate-100 border-emerald-200 text-emerald-700";
  ...
  const stopBg = isDark
    ...
    : "bg-slate-300 hover:bg-rose-500 text-slate-800 hover:text-white";
  ...
  return (
    <div
      className={`min-h-screen ${bgMain} ${textMain} font-sans flex flex-col items-center transition-colors duration-300 relative`}
    >
      ...
      <div className="max-w-7xl w-full px-4">
        <div className="text-center mb-6 md:mb-10 pt-4 flex flex-col items-center">
          ...
          <p
            className={`${textMuted} max-w-2xl mx-auto text-lg leading-relaxed`}
          >
            {t.subtitle}{" "}
            <strong
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* User Input Panel */}
          <div
            className={`${cardBg} rounded-2xl ${cardShadow} p-4 md:p-8 border ${cardBorder} backdrop-blur-md relative overflow-hidden group transition-colors duration-300`}
          >
            <div
              className={`absolute top-0 left-0 w-full h-1 bg-gradient-to-r ${isDark ? "from-emerald-500 to-teal-400 opacity-60" : "from-emerald-400 to-teal-300 opacity-80"} group-hover:opacity-100 transition-opacity`}
            ></div>
            ...
            <form onSubmit={(e) => e.preventDefault()}>
              ...
              <div className="mb-6">
                <label
                  className={`block text-xs font-bold ${textMuted} uppercase tracking-widest mb-3`}
                >
                  {t.genMode}
                </label>
                ...
                <div className="mb-6 animate-[fadeIn_0.3s_ease-out]">
                  ...
                  <div className="space-y-3">
                    <label className="flex items-center cursor-pointer group">
                      ...
                      />
                      <span
                        className={`ml-3 font-medium ${isDark ? "text-slate-300" : "text-slate-600"}`}
                      >
                        {t.usePassphrase}
                      </span>
                    </label>
                    ...
                    {usePassphrase && (
                      <div className="relative animate-[fadeIn_0.2s_ease-out]">
                        <input
                          ...
                          className={`w-full px-4 py-2.5 pr-12 ${inputBg} border ${inputBorder} rounded-lg ${inputActive} ${textMain} disabled:opacity-50 text-base transition-all shadow-sm`}
                        />
                        <button
                          type="button"
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap items-center gap-4 flex-col sm:flex-row">
                  <div
                    className={`flex w-auto items-center ${isDark ? "bg-slate-900/80 shadow-inner" : "bg-slate-50 shadow-sm"} rounded-xl p-1.5 border ${subtleBorder} select-none h-[52px]`}
                  >
                    <button
                      type="button"
                      onClick={() =>
                        setWalletCount(
                          walletCount === ""
                            ? 1
                            : Math.max(1, Number(walletCount) - 1),
                        )
                      }
                    ...
                    <div className="px-4 text-center min-w-[120px]">
                      {walletCount === "" ? (
                        <span
                          className={`text-2xl font-black ${isDark ? "text-emerald-400 drop-shadow-[0_0_8px_rgba(52,211,153,0.3)]" : "text-emerald-600"}`}
                        >
                          ∞
                        </span>
                      ) : (
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            const cappedVal =
                              val === "" ? 1 : Math.min(val, MAX_WALLETS);
                      ...
                      <span
                        className={`text-[11px] ${textMuted} block -mt-1 font-extrabold uppercase tracking-widest`}
                      >
                        {t.wallets}
                      </span>
                    </div>
                    <button
                      ...
                      onClick={() =>
                        setWalletCount(
                          walletCount === "" ? 1 : Number(walletCount) + 1,
                        )
                      }
                      disabled={running || walletCount === ""}
                      className={`w-10 h-10 flex items-center justify-center rounded-lg ${btnIconBg} text-xl transition-all hover:scale-105 active:scale-95 disabled:opacity-30 disabled:hover:scale-100 shadow-sm`}
                  ...
                  <label
                    className={`flex flex-1 sm:flex-none justify-center items-center cursor-pointer ${isDark ? "bg-slate-800/80 border-slate-700 hover:border-slate-500" : "bg-white border-slate-300 hover:border-slate-400"} px-5 py-2.5 rounded-lg border transition-all h-[52px] shadow-sm`}
                  >
                    <input
                      type="checkbox"
                      checked={walletCount === ""}
                    ...
                    />
                    <span
                      className={`ml-3 text-base font-bold ${isDark ? "text-slate-300" : "text-slate-600"}`}
                    >
                      {t.infinite}
                    </span>
                  </label>
              ...
              <div className="space-y-4 mb-6">
                <label
                  className={`block text-xs font-bold ${textMuted} uppercase tracking-widest`}
                >
                  {t.conditions}
                </label>
              ...
              <div
                className={`text-center ${textMuted} mb-6 ${isDark ? "bg-slate-900/50" : "bg-slate-100"} py-3 px-2 rounded-lg border ${subtleBorder} w-full overflow-hidden flex flex-col items-center justify-center`}
              >
                <span className="mb-1 text-sm font-semibold">{t.eg}</span>
                <span
                  className={`font-mono px-3 py-1.5 rounded border ${codeBox} break-all word-break text-[0.95rem] tracking-wide inline-block max-w-full leading-relaxed`}
                >
                  0x{conditions[0]?.prefix || ""}
                  <span className="opacity-50">{exampleHex}</span>
                  {conditions[0]?.suffix || ""}
                </span>
              </div>
              ...
              <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
                <label
                  className={`flex items-center cursor-pointer group-hover:${isDark ? "text-amber-200" : "text-amber-600"} transition-colors`}
                >
                  <input
                    type="checkbox"
                    checked={checksum}
                ...
                <div className="flex flex-col items-end gap-2">
                  <div
                    className={`flex w-full sm:w-auto items-center ${isDark ? "bg-slate-900/80 shadow-inner" : "bg-slate-50 shadow-sm"} rounded-xl p-1.5 border ${subtleBorder} select-none h-[52px]`}
                  >
                    <button
                      type="button"
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        setThreads(Math.min(maxCores, threads + 1))
                      }
                  ...
                  <div className="flex gap-2.5 w-full justify-center items-center">
                    <button
                      ...
                      disabled={running}
                      onClick={() =>
                        setThreads(Math.max(1, Math.floor(maxCores / 3)))
                      }
                      className={`text-[12px] uppercase tracking-widest font-black px-3 py-2 rounded-lg border ${isDark ? "border-slate-700 bg-slate-800/50 hover:bg-slate-700 text-slate-400" : "border-slate-300 bg-white hover:bg-slate-100 text-slate-500 shadow-sm"} transition-all hover:scale-105 active:scale-95 disabled:opacity-30 disabled:hover:scale-100`}
                    >
                      Low
                    </button>
                    <button
                      ...
                      disabled={running}
                      onClick={() =>
                        setThreads(Math.max(1, Math.floor(maxCores / 2)))
                      }
                      className={`text-[12px] uppercase tracking-widest font-black px-3 py-2 rounded-lg border ${isDark ? "border-slate-700 bg-slate-800/50 hover:bg-slate-700 text-slate-400" : "border-slate-300 bg-white hover:bg-slate-100 text-slate-500 shadow-sm"} transition-all hover:scale-105 active:scale-95 disabled:opacity-30 disabled:hover:scale-100`}
                    >
                      Medium
                    ...
                    <button
                      type="button"
                      disabled={running}
                      onClick={() => setThreads(maxCores)}
                      className={`text-[12px] uppercase tracking-widest font-black px-3 py-2 rounded-lg border ${isDark ? "border-emerald-700/50 bg-emerald-900/20 hover:bg-emerald-800/40 text-emerald-400" : "border-emerald-300 bg-emerald-50 hover:bg-emerald-100 text-emerald-600 shadow-sm"} transition-all hover:scale-105 active:scale-95 disabled:opacity-30 disabled:hover:scale-100`}
                    >
                      High
                    </button>
                  </div>
          ...
          {/* Statistics Panel */}
          <div
            className={`${cardBg} rounded-2xl ${cardShadow} p-4 md:p-8 border ${cardBorder} backdrop-blur-md relative overflow-hidden group flex flex-col justify-between transition-colors duration-300`}
          >
            <div
              className={`absolute top-0 right-0 w-full h-1 bg-gradient-to-l ${isDark ? "from-emerald-500 to-teal-400 opacity-60" : "from-emerald-400 to-teal-300 opacity-80"} group-hover:opacity-100 transition-opacity`}
            ></div>

            <div className={`space-y-4 ${textMain}`}>
              <div
                className={`flex justify-between items-end border-b ${subtleBorder} pb-3`}
              >
                <span
                  className={`${textMuted} text-sm uppercase tracking-wider font-bold`}
                >
                  {t.difficulty || "Difficulty"}
                </span>
                <span
                  className={`font-mono ${isDark ? "text-emerald-400" : "text-emerald-600"} truncate max-w-[60%] text-right font-medium`}
                >
                  {displayDifficulty}
                </span>
              </div>
              ...
              >
                ...
                <span
                  className={`font-mono ${isDark ? "text-emerald-400" : "text-emerald-600"} font-medium`}
                >
                  {(foundWallets || []).length}{" "}
                  <span
                    className={`${textMuted} text-sm font-sans tracking-normal lowercase`}
                  >
                    {t.wallets || "wallets"}
                  </span>
                </span>
              ...
              >
                ...
                <span
                  className={`font-mono ${isDark ? "text-amber-400" : "text-amber-600"} font-medium`}
                >
                  {displayTime50}
                </span>
              </div>
              ...
              >
                ...
                >
                  ...
                  <span
                    className={`font-mono ${isDark ? "text-indigo-400" : "text-indigo-600"} font-medium`}
                  >
              ...
              {foundWallets.length > 0 && (
                <>
                  ...
                  >
                    ...
                    >
                      {foundWallets.length > 0
                        ? `${(timeElapsed / foundWallets.length).toFixed(1)}s`
                        : t.na}
                    </span>
                  </div>
                  <div
                  ...
                  >
                    ...
                    >
                      {foundWallets.length > 0
                        ? formatNum(Math.floor(attempts / foundWallets.length))
                        : t.na}
                    </span>
                  </div>
                </>
            ...
            <div className="mt-8 flex justify-between items-end">
              <div className="w-full mr-4 relative">
                <div
                  className={`w-full ${isDark ? "bg-slate-900" : "bg-slate-200"} rounded-lg h-5 overflow-hidden border ${isDark ? "border-slate-700/80 shadow-inner" : "border-slate-300 shadow-inner"}`}
                >
                  <div
              ...
              <div className="text-right whitespace-nowrap hidden sm:block">
                <span
                  className={`block text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r ${isDark ? "from-emerald-400 to-teal-300" : "from-emerald-600 to-teal-500"} leading-none pb-1`}
                >
                  {probabilityLabel}
                </span>
                <span
                  className={`block text-xs ${textMuted} font-bold uppercase tracking-widest mt-0.5`}
                >
                  {t.probability}
                </span>
              </div>
            ...
            <div className="text-center sm:hidden mt-3">
              <span
                className={`text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r ${isDark ? "from-emerald-400 to-teal-300" : "from-emerald-600 to-teal-500"}`}
              >
                {probabilityLabel}
              </span>
              <span
                className={`ml-2 text-xs ${textMuted} font-bold uppercase tracking-widest`}
              >
                {t.probability}
              </span>
            </div>
        ...
        {foundWallets.length > 0 && (
          <div
            className={`mt-8 ${cardBg} rounded-2xl ${isDark ? "shadow-[0_0_40px_rgba(16,185,129,0.15)]" : "shadow-xl border-emerald-300"} p-4 md:p-8 border ${isDark ? "border-emerald-500/50" : "border-emerald-400"} backdrop-blur-sm animate-[fadeIn_0.5s_ease-out] transition-colors duration-300`}
          >
            <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
              <h2
                className={`text-2xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r ${isDark ? "from-emerald-400 to-teal-300" : "from-emerald-600 to-teal-500"} flex items-center gap-3`}
              >
            ...
            <div className="max-h-[600px] overflow-y-auto py-2 pr-2 custom-scrollbar space-y-3 sm:space-y-6">
              {foundWallets.slice(0, displayCount).map((w, idx) => (
                <div
                  key={w.id}
                  className={`p-4 md:p-6 rounded-2xl border ${isDark ? "border-slate-700/50 bg-slate-900/40" : "border-slate-200 bg-slate-50/50"} relative group/item`}
                >
                  <div
                    className={`absolute -top-2 left-3 min-w-9 h-9 p-2 rounded-full ${isDark ? "bg-emerald-600" : "bg-emerald-500"} text-white flex items-center justify-center font-bold text-sm shadow-lg`}
                  >
                    {idx + 1}
                  </div>
                  ...
                  <div className="grid gap-4">
                    <div
                      className={`${successBox} p-3 md:p-5 rounded-xl shadow-inner group transition-colors ${isDark ? "hover:border-emerald-500/30" : "hover:border-emerald-400"}`}
                    >
                      <div className="flex justify-between items-center mb-2">
                        <label
                          className={`text-xs font-bold ${isDark ? "text-emerald-400" : "text-emerald-600"} uppercase tracking-widest`}
                        >
                          {idx === 0
                            ? t.ethAddress
                            : `${t.ethAddress} #${idx + 1}`}
                    ...
                    <div
                      className={`${successBox} p-3 md:p-5 rounded-xl shadow-inner group transition-colors ${isDark ? "hover:border-rose-500/30" : "hover:border-rose-400"} relative overflow-hidden`}
                    >
                      <div className="flex justify-between items-center mb-2 relative z-10">
                        <label
                          className={`text-xs font-bold ${isDark ? "text-rose-400" : "text-rose-600"} uppercase tracking-widest flex items-center gap-2`}
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            className="h-4 w-4"
                        ...
                        <div className="flex gap-2">
                          <button
                            onClick={() =>
                              copyToClipboard(w.privKey, t.copiedPrivKey)
                            }
                      ...
                      <div
                        className={`font-mono text-sm sm:text-base break-all transition-all duration-300 relative font-medium z-10 ${
                          w.isKeyVisible
                            ? `${privKeyText} select-all`
                            : `${isDark ? "text-rose-500/50" : "text-rose-400/50"} blur-sm select-none`
                        }`}
                      >
                    ...
                    {w.mnemonic && (
                      <div
                        className={`${successBox} p-3 md:p-5 rounded-xl shadow-inner group transition-colors ${isDark ? "hover:border-indigo-500/30" : "hover:border-indigo-400"} relative overflow-hidden`}
                      >
                        <div className="flex justify-between items-center mb-2 relative z-10">
                          <label
                            className={`text-xs font-bold ${isDark ? "text-indigo-400" : "text-indigo-600"} uppercase tracking-widest flex items-center gap-2`}
                          >
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              className="h-4 w-4"
                          ...
                          <div className="flex gap-2">
                            ...
                            <button
                              onClick={() =>
                                toggleWalletVisibility(w.id, "mnemonic")
                              }
                              className={`text-slate-400 ${isDark ? "hover:text-amber-400 bg-slate-800 hover:bg-slate-700 border-slate-700/50" : "hover:text-amber-500 bg-slate-100 hover:bg-slate-200 border-slate-200"} transition-colors p-1.5 rounded-lg border`}
                            >
                              {w.isMnemonicVisible ? (
                        ...
                        <div
                          className={`font-mono text-sm sm:text-base break-all transition-all duration-300 relative font-medium z-10 ${
                            w.isMnemonicVisible
                              ? `text-indigo-400 select-all`
                              : `${isDark ? "text-indigo-500/50" : "text-indigo-400/50"} blur-sm select-none`
                          }`}
                        >
            ...
            <p
              className={`text-rose-500/90 text-sm font-semibold text-center mt-4`}
            >
              {t.securityWarning}
            </p>
          </div>
        ...
        {/* Disclaimer */}
        <div
          className={`mt-8 md:mt-12 ${isDark ? "bg-slate-900/60 border-amber-900/40 shadow-xl" : "bg-amber-50/80 border-amber-200 shadow-md"} rounded-xl flex flex-col sm:flex-row p-4 md:p-6 border mx-auto backdrop-blur-sm transition-colors duration-300`}
        >
          ...
          <div>
            <h3 className="font-bold text-amber-500 mb-2 uppercase tracking-wide">
              {t.securityNoticeTitle}
            </h3>
            <p
              className={`text-sm ${isDark ? "text-slate-400" : "text-slate-600"} leading-relaxed font-medium`}
            >
        ...
        {/* Footer */}
        <footer
          className={`mt-10 md:mt-16 py-4 md:py-6 border-t ${isDark ? "border-slate-800" : "border-slate-200"} flex flex-col md:flex-row justify-between items-center gap-6 mx-auto w-full transition-colors duration-300`}
        >
          ...
          <div
            className={`flex items-center gap-6 text-sm font-bold ${isDark ? "text-slate-400" : "text-slate-500"}`}
          >
            <a
      ...
      {showKeystoreModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-[fadeIn_0.2s_ease-out]">
          <div
            className={`${isDark ? "bg-slate-800 border-slate-700 text-slate-200" : "bg-white border-slate-200 text-slate-800"} border rounded-2xl shadow-2xl p-6 md:p-8 w-full max-w-md relative`}
          >
            <button
            ...
            <h3
              className={`text-xl font-bold mb-2 ${isDark ? "text-indigo-400" : "text-indigo-600"} flex items-center gap-2`}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-6 w-6"
            ...
            >
              ...
              {isEncrypting ? (
                <div
                  className={`w-full py-6 rounded-2xl border transition-all flex flex-col items-center justify-center gap-4 ${isDark ? "bg-slate-800/50 border-slate-700" : "bg-slate-50 border-slate-200"}`}
                >
                  <div className="flex flex-col items-center gap-3 px-4 text-center">
                    <div
                      className={`p-3 rounded-full ${isDark ? "bg-indigo-500/20 text-indigo-400" : "bg-indigo-100 text-indigo-600"}`}
                    >
                      <svg
        ...
        >
          ...
          >
            ...
            <h3 className="text-2xl font-bold mb-4 text-center">
              {t.infiniteWarningTitle}
            </h3>
            <p
              className={`text-center mb-6 ${isDark ? "text-slate-400" : "text-slate-600"} leading-relaxed`}
            >
            ...
            <label className="flex items-center justify-center mb-8 cursor-pointer group">
              ...
              />
              <span
                className={`ml-3 font-medium ${isDark ? "text-slate-300" : "text-slate-600"} group-hover:text-emerald-500 transition-colors`}
              >
                {t.dontShowAgain}
              </span>
            </label>
